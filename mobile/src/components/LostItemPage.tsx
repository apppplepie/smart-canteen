...
import { ChevronLeft, Image as ImageIcon, MapPin, Clock, Search, CheckCircle2 } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { cn } from "../lib/utils";
import { SharedPostDetail } from "./SharedPostDetail";

export function LostItemPage({ onBack }: { onBack: () => void }) {
  ...
  const toggleFound = (id: number, e: React.MouseEvent) => {
    e.stopPropagation();
    setHistory(history.map(h => h.id === id ? { ...h, isFound: !h.isFound } : h));
  };
