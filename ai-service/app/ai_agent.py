...
# mobile 允许查表：商家、菜品、订单、订单明细、排队，用于点餐/食堂问答
...
# mobile 已登录时由 get_tools_for_request 用闭包工具替换为「仅自己的订单 + 仅当天排队」；不开放用户表
TOOLS_FOR_MOBILE = [
    get_order_info,
    list_orders_by_user,
    list_posts,
...
# mobile 未传 user_id 时不给订单/排队查表，避免越权；且不开放用户表查询
TOOLS_FOR_MOBILE_NO_USER = [
    get_order_info,
    list_orders_by_user,
    list_posts,
    TOOLS_BY_NAME["query_vendors"],
...
请保持像朋友聊天一样自然、友好、简洁，不要使用表情包、emoji、颜文字，也不要写括号里的补充说明。若问题与食堂无关，可委婉引导回食堂话题。"""

MOBILE_SYSTEM_PROMPT = """你是大学食堂的智能点餐助手，语气自然、友好，像朋友聊天一样。
不要使用表情包、emoji、颜文字，也不要写括号里的补充说明。
...
def _make_mobile_scoped_tools(user_id: int | None) -> list:
    """
    Mobile 端已登录时：仅能查「自己的」订单/订单明细，排队仅查「当天」。
    返回三个闭包工具，避免 token 爆炸与越权。
    """
    ...
    def _query_queue_entries_today(page: int = 1, size: int = 10) -> str:
        return _get(f"/api/v1/data/queue_entries?currentPage={page}&size={size}&todayOnly=true")

    return [
        StructuredTool.from_function(
            name="query_my_orders",
            description="查询当前登录用户自己的订单列表（分页）。page 从 1 开始，size 每页条数（最大 100）。",
            func=_query_my_orders,
        ),
        StructuredTool.from_function(
            name="query_my_order_items",
            description="查询当前登录用户自己的订单明细列表（分页）。page 从 1 开始，size 每页条数（最大 100）。",
            func=_query_my_order_items,
        ),
        StructuredTool.from_function(
            name="query_queue_entries_today",
            description="仅查询当天的排队记录（分页），用于回答今日排队情况。page 从 1 开始，size 每页条数（最大 100）。",
            func=_query_queue_entries_today,
        ),
    ]


def get_tools_for_request(client_type: str, role: str | None, user_id: int | None = None) -> list:
    """
    根据 clientType、role、user_id 决定本次请求可用的工具。
    - admin：按 role 白名单。
    - screen：TOOLS_FOR_SCREEN
    - mobile：若传 user_id 则用「仅查自己订单/订单明细 + 仅查当天排队」的闭包工具；否则用 TOOLS_FOR_MOBILE（不含订单/排队查表）
    """
...
def process_message(
    ...
    """
    统一入口：根据 clientType、role、user_id 选工具，执行 Agent 或仅 LLM 回复。
    messages: 最近几轮对话 [{"role":"user"/"assistant","content":"..."}]，至少一条 user。
    context_summary: 可选，此前对话的 500 字内小结。
    user_id: 可选，mobile 登录用户 ID，用于仅查自己的订单/订单明细与仅查当天排队。
